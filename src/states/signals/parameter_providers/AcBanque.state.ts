...
class AcBanqueStateProvider extends ICrudStateProvider<AcBanque> {
    ...
    getBanqueFromId = (banqCode: string | undefined): AcBanque | undefined => {
        return (this.getState().value as AcBanque[]).find((banq) => banq.code.toLowerCase().trim() == banqCode?.toLowerCase().trim())
    }
}

...
const CreanceTabsView = ({ tabs, state }: CreanceTabsViewProps) => {
    ...
    const buildTableContent = (tabs: CreanceTabType[]): JSX.Element => {
        ...
        return <TabPanels>
            {tabs.map(({ tableContent, fields, tableHeaders, rowCount, additionnalContents, hasAddButton }: CreanceTabType, index) => <BuildTabContent index={index} tableHeaders={tableHeaders} tableContent={tableContent} hasAddButton={hasAddButton} fields={fields} rowCount={rowCount} additionnalContents={additionnalContents} />
            )}
        </TabPanels>
